...
public class CoroutineManagement : SingletonBehavior<CoroutineManagement>
{
    ...
    public IEnumerator StartManagedCoroutine(IEnumerator coroutine, float delaySeconds = 0f, Action onComplete = null)
    {
        if (delaySeconds < 0)
        {
            throw new ArgumentException("DelaySeconds cannot be negative.", nameof(delaySeconds));
        }

        if (_coroutines.TryGetValue(coroutine, out var existingCoroutine) &&
            existingCoroutine.State != CoroutineState.Finished)
        {
            return coroutine;
        }
